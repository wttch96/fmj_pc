...
public class ResMap extends ResBase {
  ...
  /** 地图数据 两个字节表示一个地图快（从左到右，从上到下） （低字节：最高位1表示可行走，0不可行走。高字节：事件号） */
  private byte[] mData;
  ...
  public void setData(byte[] buf, int offset) {
    ...
    System.out.println(mName + ":[" + mWidth + "/" + mHeight + "]");
    for (int y = 0; y < mHeight; y++) {
      for (int x = 0; x < mWidth; x++) {
        System.out.print(String.format("[%03d] ", getTileIndex(x, y)));
      }
      System.out.println();
    }
    for (int y = 0; y < mHeight; y++) {
      for (int x = 0; x < mWidth; x++) {
        int event = getEventNum(x, y);
        if (event > 0) {
          System.out.println(String.format("[%d,%d]有事件[%d]", x, y, event));
        }
      }
    }
  }
  ...
  public boolean canPlayerWalk(int x, int y) {
    return canWalk(x, y) && (x >= 4) && (x < mWidth - 4) && (y >= 3) && (y < mHeight - 2);
  }
  ...
  public void drawMap(Canvas canvas, int left, int top) {
    ...
    for (int y = 0; y < minY; y++) {
      for (int x = 0; x < minX; x++) {
        mTiles.draw(
            canvas,
            x * Tiles.WIDTH + Global.MAP_LEFT_OFFSET,
            y * Tiles.HEIGHT,
            getTileIndex(left + x, top + y));
      }
    }
  }
