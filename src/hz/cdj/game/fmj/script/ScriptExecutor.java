...
public class ScriptExecutor {
  ...
  /** mEventIndex[i]等于触发事件i+1时，要执行的Operate在list中的序号， -1表示不存在 */
  ...
  public ScriptExecutor(
      ArrayList<Operate> list, int[] eventIndex, HashMap<Integer, Integer> map, int headerCnt) {
    mOperateList = list;
    ...
    for (int i = 0; i < mEventIndex.length; i++) {
      if (mEventIndex[i] != -1) {
        System.out.println(i + "/" + mEventIndex[i]);
        System.out.println(mOperateList.get(mEventIndex[i]));
      }
    }
  }
